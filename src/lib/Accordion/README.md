...
| `size` | `object` | `-` | Размеры сетки содержимого (количество колонок и строк) |
| `size.height` | `number` | `1` | Количество строк в сетке содержимого |
| `size.width` | `number` | `1` | Количество колонок в сетке содержимого |
| `label` | `object` | `-` | Настройки заголовка аккордеона |
| `label.name` | `string` | `""` | Текст заголовка аккордеона |
| `label.class` | `string` | `"text-left"` | CSS-классы для стилизации текста заголовка |
| `label.icon` | `ConstructorOfATypedSvelteComponent \| string \| null` | `null` | Иконка, отображаемая в заголовке (SVG строка или компонент) |
