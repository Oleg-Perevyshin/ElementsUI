...
<div class={twMerge(`flex w-full flex-col gap-1`, wrapperClass)}>
  ...
  <!-- Такая же зарезервированная строка, как у Input (h-4, всегда пустая — у Select нет
       валидации), чтобы высота совпадала при совместном размещении в одной строке (items-end). -->
  <span class="block h-4 text-[12px]"></span>
</div>
