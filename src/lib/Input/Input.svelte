...
<div class={twMerge(`relative flex w-full flex-col gap-1 ${type === "text-area" ? "h-full" : ""}`, wrapperClass)}>
  ...
  {#if help.regExp}
    <!-- Место под ошибку зарезервировано всегда (высота фиксирована), чтобы её появление
         не двигало соседние элементы — кнопку в одной строке с полем (как в восстановлении
         пароля) или соседние поля формы. -->
    <span class="block h-4 text-[12px] text-(--red-color)">{!isValid ? "Неверный формат" : ""}</span>
  {/if}
</div>
