...
<div class={twMerge(`relative flex w-full flex-col gap-1 ${type === "text-area" ? "h-full" : ""}`, wrapperClass)}>
        ...
        {#if showPassword}
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"
            ><g fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
              ><path d="M15 12a3 3 0 1 1-6 0a3 3 0 0 1 6 0" /><path d="M2 12c1.6-4.097 5.336-7 10-7s8.4 2.903 10 7c-1.6 4.097-5.336 7-10 7s-8.4-2.903-10-7" /></g
            ></svg
          >
        {:else}
          <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"
  ...
  {#if help.regExp && !isValid}
    <!-- Абсолютное позиционирование — сообщение об ошибке не должно занимать место в потоке
       и сдвигать соседние поля/кнопки в общей строке; было: зарезервированная строка h-4
       всегда в потоке — давала лишний зазор везде, где ошибке взяться неоткуда (например
       вертикальный список кнопок быстрых команд в Отладке). -->
    <span class="absolute top-full left-0 mt-1 text-[12px] text-(--red-color)">Неверный формат</span>
  {/if}
</div>
