...
<div class={twMerge(`relative flex w-full flex-col gap-1 ${type === "text-area" ? "h-full" : ""}`, wrapperClass)}>
  ...
  <!-- Место под ошибку зарезервировано всегда (высота фиксирована, не только когда задан
       help.regExp), чтобы Input сохранял одну и ту же высоту во всех случаях — иначе он не
       совпадает по высоте с Select в одной строке (items-end), когда у одного поля есть
       валидация, а у другого нет. -->
  <span class="block h-4 text-[12px] text-(--red-color)">{help.regExp && !isValid ? "Неверный формат" : ""}</span>
</div>
